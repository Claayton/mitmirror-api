"""Testes para a classe GetUser"""  # pylint: disable=E0401
...
def test_by_id(get_user, user_repository_spy, fake_user):
    """
    Testando o metodo by_id.
    Deve retornar uma mensagem de sucesso, e um usuario.
    """

    response = get_user.by_id(user_id=fake_user.id)
...
def test_by_email(get_user, user_repository_spy, fake_user):
    """
    Testando o metodo by_email.
    Deve retornar uma mensagem de sucesso, e um usuario.
    """

    response = get_user.by_email(email=fake_user.email)
...
def test_by_email_with_invalid_param(get_user, user_repository_spy, fake_user):
    """
    ...
    response = get_user.by_email(email=fake_user.id)
...
def test_by_username(get_user, user_repository_spy, fake_user):
    """
    Testando o metodo by_username.
    Deve retornar uma mensagem de sucesso, e um usuario.
    """

    response = get_user.by_username(username=fake_user.username)
...
def test_by_username_with_invalid_param(get_user, user_repository_spy, fake_user):
    """
    ...
    response = get_user.by_username(username=fake_user.id)
